  ...
  methods: {
    ...
    addAttrValue() {
      ...
      this.attrInfo.attrValueList.push({
        attrId: this.attrInfo.id, //对于修改某个属性的时候，可以在已有的属性的基础之上新增新的属性值（需要把已有的属性的id带上）
        valueName: "",
        // flag属性：给每一个属性值添加一个标记flag，用于切换查看模式与编辑模式，好处：每一个属性值可以控制自己的模式进行切换
    ...
    toLook(row) {
      // 如果属性值为空不能作为新的属性值，需要给用户提示，让她输入一个其他的属性值
    ...
    toEdit(row, index) {
      row.flag = true;
      // 获取input节点，实现自动聚焦
      // 点击span的时候，重绘一个input需要耗费时间，因此我们不可能一点击span立马获取到input
